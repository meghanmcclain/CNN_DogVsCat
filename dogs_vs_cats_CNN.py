...
training_set = training_data_generator.flow_from_directory(src + 'Train/', target_size = (INPUT_SIZE, INPUT_SIZE), batch_size = BATCH_SIZE, class_mode = 'binary')
# Train the model
print("Training the CNN...")
model.fit(training_set, steps_per_epoch = STEPS_PER_EPOCH, epochs = EPOCHS, verbose=1) #was told to replace model.fit_generator() with model.fit()
######################################################################
##### TESTING                                                    #####
######################################################################
# Create ImageDataGenerator to load batches of images at a time into memory
...
score = model.evaluate(test_set, steps=100) #was told to replace model.evaluate_generator() with model.evaluate()
######################################################################
##### RESULTS                                                    #####
######################################################################
# Display the results
